...
(define (gnc:html-chart-set-tooltip-mode! chart mode)
  (gnc:html-chart-set! chart '(options plugins tooltip mode) mode))

(define (gnc:html-chart-set-tooltip-caretpadding! chart padding)
  (gnc:html-chart-set! chart '(options plugins tooltip caretPadding) padding))
...
(define (gnc:html-chart-set-tooltip-non-zero-only! chart nonZeroOnly)
  (gnc:html-chart-set! chart '(options plugins tooltip showNonZeroOnly) nonZeroOnly))
...
(define (apply-report-preferences! chart)
  ...
  (gnc:html-chart-set! chart '(options plugins tooltip caretSize)
                       (gnc-prefs-get-int "general.report" "chart-tooltip-caret-size"))

  (gnc:html-chart-set! chart '(options plugins tooltip position)
...
function tooltipFilter(context) {
...
(define (gnc:html-chart-render chart doc)
  ...
  (let* ((retval '())
    ...
    (push "if (chartjsoptions.options.plugins.tooltip.showNonZeroOnly) { chartjsoptions.options.plugins.tooltip.filter = tooltipFilter; }\n")
    (push JS-setup)
