...
SCM gnc_qrcode_encodestring (gchar *str)
{
    ...
    qr = QRcode_encodeString (str, 0, QR_ECLEVEL_M, QR_MODE_8, FALSE);
    ...
    for (gchar *p = qr->data; *p; p++)
        output = scm_cons ((*p & 1) ? SCM_BOOL_T : SCM_BOOL_F, output);

    output = scm_reverse (output);
