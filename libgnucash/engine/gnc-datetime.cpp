    ...
    m_time(unix_epoch, utc_zone)
{
    ...
    try
    {
        ...
        static constexpr ctll::fixed_string delim_iso("^(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}(?:\\.\\d{0,9})?)\\s*([+\\-]\\d{2}(?::?\\d{2})?)?$");
        static constexpr ctll::fixed_string non_delim("^(\\d{14}(?:\\.\\d{0,9})?)\\s*([+\\-]\\d{2}\\s*(:?\\d{2})?)?$");
        PTime pdt;
        std::string tzstr;
        if (auto sm = ctre::match<non_delim>(str))
        {
