...
(define (build-column-used options)   
  ...
  (let* ((col-vector (make-vector columns-used-size #f))
    ...
    (set-col (if (gnc:lookup-option options "Display" "Num")
                 (opt-val "Display" "Num")
                 (opt-val "Display" "Num/Action"))
             1)
    (set-col (if (opt-val "__reg" "journal")
                 (or (opt-val "Display" "Memo")
                     (opt-val "Display" "Description")
                     (opt-val "__reg" "double"))
                 (opt-val "Display" "Description"))
             2)
    (set-col (and (not (opt-val "__reg" "journal"))
...
(define (make-split-table splits options
  ...
  (let* ((table (gnc:make-html-table))
    ...
    ;; main loop
    ;; ----------------------------------
    (let do-rows-with-subtotals ((splits splits)
                                 (odd-row? #t)
      ...
      (if (null? splits)
          ...
          (let* ((current (car splits))
                 ...
                 (valid-split? (pair? (xaccSplitGetAccount current))))
            ;; ----------------------------------------------
