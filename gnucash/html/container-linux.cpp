/*******************************************************************\
 * container-linux.cpp -- litehtml container linux document         *
 *                                                                  *
...
void container_linux::draw_background(litehtml::uint_ptr hdc, const std::vector<litehtml::background_paint>& bgvec)
{
    ...
    for (int i = (int)bgvec.size() - 1; i >= 0; i--)
    {
        ...
        if(img_i != m_images.end() && img_i->second)
        {
            ...
            {
                GdkPixbuf* new_img = gdk_pixbuf_scale_simple(bgbmp, bg.image_size.width, bg.image_size.height, GDK_INTERP_BILINEAR);
                bgbmp = new_img;
            }
...
cairo_surface_t* container_linux::surface_from_pixbuf(const GdkPixbuf* bmp)
{
    ...
    gdk_cairo_set_source_pixbuf(ctx, bmp, 0.0, 0.0);
    cairo_paint (ctx);
...
void container_linux::get_media_features(litehtml::media_features& media) const
{
    ...
    media.device_width  = geometry.width;
    media.device_height = geometry.height;
    media.color         = 8;
    media.monochrome    = 0;
