                               ...
                               const char *key)
{
    ...
    if (!category)
        kvp_path = g_strdup_printf ("%s/%s", IMAP_FRAME, key);
    else
        kvp_path = g_strdup_printf ("%s/%s/%s", IMAP_FRAME, category, key);
                                 ...
                                 const char *key)
{
    ...
    if (qof_instance_has_slot (QOF_INSTANCE (imap->acc), kvp_path))
    {
        ...
        if (category)
        {
            kvp_path = g_strdup_printf ("%s/%s", IMAP_FRAME, category);
                                    ...
                                    Account *acc)
{
            ...
            current_token = current_token->next)
    {
        ...
        kvp_path = g_strdup_printf ("%s/%s/%s", IMAP_FRAME_BAYES,
                                    (char*)current_token->data,
                                    guid_string);
...
//FIXME    change_imap_entry (imap, kvp_path, token_count);
