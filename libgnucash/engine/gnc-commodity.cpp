...
gnc_commodity_namespace_get_commodity_list(const gnc_commodity_namespace *name_space)
{
    ...
    return std::accumulate (name_space->cm_table.begin(), name_space->cm_table.end(),
                            static_cast<GList*>(nullptr),
                            [](auto prev, auto pair){ return g_list_prepend (prev, pair.second); });
}
                           ...
                           const char * name_space, const char * mnemonic)
{
    ...
    if (nsp)
    {
        ...
        auto it = nsp->cm_table.find(mnemonic);
        return it == nsp->cm_table.end() ? nullptr : it->second;
    }
    else
    {
        return nullptr;
                                       ...
                                       gpointer user_data)
{
    ...
    auto comm_func = [&](const auto& n){ return f(n.second, user_data); };
    auto ns_func = [&](const auto& ns)
    { return std::all_of (ns.second->cm_table.begin(), ns.second->cm_table.end(), comm_func); };
    return std::all_of (tbl->ns_table.begin(), tbl->ns_table.end(), ns_func);
}
...
gnc_commodity_table_destroy(gnc_commodity_table * t)
{
    ...
    while (!t->ns_table.empty())
    {
        auto first_ns{t->ns_table.begin()->second};
        gnc_commodity_table_delete_namespace (t, first_ns->name);
    }
