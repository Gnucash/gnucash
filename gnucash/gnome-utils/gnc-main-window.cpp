...
{
    /* File menu */
    { "FileImportAction", nullptr, N_("_Import"), nullptr, nullptr },
    { "FileExportAction", nullptr, N_("_Export"), nullptr, nullptr },
    {
        "FilePrintAction", "document-print", N_("_Print..."), "<primary>p",
        N_("Print the currently active page")
    },
...
#    define GTK_STOCK_PAGE_SETUP nullptr
    ...
    /* View menu */
    { "ViewTabPositionAction", nullptr, N_("Tab P_osition"), nullptr, nullptr },
    {
        "ViewSortByAction", nullptr, N_("_Sort By..."), nullptr,
        N_("Select sorting criteria for this page view")
    },
    {
        "ViewFilterByAction", nullptr, N_("_Filter By..."), nullptr,
        N_("Select the account types that should be displayed.")
    },
    {
    ...
    /* Actions menu */
    { "ScrubMenuAction", nullptr, N_("_Check & Repair"), nullptr, nullptr },
    {
    ...
    },
    {
        ...
        N_("Display the notebook tabs at the top of the window.")
    },
    {
        "ViewTabPositionBottomAction", nullptr, N_("B_ottom"), nullptr,
        N_("Display the notebook tabs at the bottom of the window.")
    },
    {
        "ViewTabPositionLeftAction", nullptr, N_("_Left"), nullptr,
        N_("Display the notebook tabs at the left of the window.")
    },
    {
        "ViewTabPositionRightAction", nullptr, N_("_Right"), nullptr,
        N_("Display the notebook tabs at the right of the window.")
    },
};
                                        ...
                                        struct menu_update *data)
{
    ...
    action_name = g_strdup_printf ("Window%02dAction", data->index);

    menu_item =  gnc_find_menu_item (priv->menubar, action_name);
    ...
    gtk_menu_item_set_label (GTK_MENU_ITEM(menu_item), data->label);
    gtk_widget_set_visible (menu_item, data->visible);
    g_free (action_name);
...
gnc_main_window_update_menu_item (GncMainWindow *window)
{
    ...
    data.visible = TRUE;
    data.action_name = g_strdup ("WindowAction");
    data.index = index;
...
gnc_main_window_update_all_menu_items (void)
{
    ...
    data.visible = FALSE;
    for (gsize i = g_list_length (active_windows); i < gnc_main_window_max_number; i++)
    {
        data.index = i;
        data.action_name = g_strdup_printf ("Window%02dAction", data.index);
        data.label = g_strdup_printf ("Window _%" G_GSIZE_FORMAT, i % 10);
                               ...
                               gpointer user_data)
{
    ...
    entry->simple_action_group = g_simple_action_group_new ();

    g_action_map_add_action_entries (G_ACTION_MAP(entry->simple_action_group),
                                     actions,
                                     n_actions,
                                     data);
                                     ...
                                     gint n_display_items)
{
    ...
    g_return_if_fail (GNC_IS_MAIN_WINDOW (window));
    g_return_if_fail (display_items != nullptr || n_display_items == 0);
                               ...
                               gint n_updates)
{
    ...
    // Edit Preferences
    auto item = gnc_find_menu_item (priv->menubar, "EditPreferencesAction");
    if (item)
    {
...
gnc_main_window_init_menu_updaters (GncMainWindow *window)
{
    ...
    edit_menu_item = gnc_find_menu_item (priv->menubar, "EditAction");
...
gnc_main_window_setup_window (GncMainWindow *window)
{
    ...
    extra_item = gnc_find_menu_item (priv->menubar, "ExtensionsAction");
    gtk_widget_set_visible (extra_item, gnc_prefs_is_extra_enabled());
...
gnc_quartz_set_menu (GncMainWindow* window)
{
    ...
    // File Quit
    item = gnc_find_menu_item (priv->menubar, "FileQuitAction");
    if (item)
        gtk_widget_hide (GTK_WIDGET(item));

    // Help About
    item = gnc_find_menu_item (priv->menubar, "HelpAboutAction");
    if (item)
    {
    ...
    // Help Menu
    item = gnc_find_menu_item (priv->menubar, "HelpAction");
    if (item)
        gtkosx_application_set_help_menu (theApp, GTK_MENU_ITEM(item));
    // Windows Menu
    item = gnc_find_menu_item (priv->menubar, "WindowsAction");
    if (item)
        gtkosx_application_set_window_menu (theApp, GTK_MENU_ITEM(item));

    for (gint i = 0; i < gnc_main_window_max_number; i++)
    {
        GtkWidget *menu_item;
        gchar *action_name = g_strdup_printf ("Window%02dAction", i);
        menu_item = gnc_find_menu_item (priv->menubar, action_name);
        gtk_widget_hide (menu_item);
    }
