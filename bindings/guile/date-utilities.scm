...
(define (gnc:time64-now)
  ;; Guile's current-time can be backed by a 32-bit time_t on Windows. Keep
  ;; all report-relative dates on the engine's time64 clock instead.
  (gnc-time 0))
...
(define (make-zdate) 
  (let ((zd (gnc-localtime (gnc:time64-now))))
    (set-tm:hour zd 0)
    (set-tm:min zd 0)
    (set-tm:sec zd 0)
    (set-tm:mday zd 0)
...
(define (gnc:get-start-cal-year)
  (let ((now (gnc-localtime (gnc:time64-now))))
    (set-tm:sec now 0)
    (set-tm:min now 0)
    (set-tm:hour now 0)
    (set-tm:mday now 1)
