...
day_selected (GtkCalendar *calendar, GNCDateEdit *gde)
{
    ...
    t = gnc_dmy2time64 (day, month, year);
...
static gboolean
day_selected_double_click (GtkGestureClick *gesture,
                           int n_press,
                           double x,
                           double y,
                           gpointer user_data)
...
static gint
delete_popup (GtkWidget *widget, gpointer user_data)
{
    GNCDateEdit *gde = user_data;
                 ...
                 gpointer user_data)
{
    ...
    GdkEvent *event = gtk_event_controller_get_current_event (GTK_EVENT_CONTROLLER(key));
    ...
    g_signal_stop_emission_by_name (G_OBJECT(widget), "key-pressed");
    gnc_date_edit_popdown (gde);
...
gnc_date_edit_popup (GNCDateEdit *gde)
{
    ...
    GDateTime *date = g_date_time_new (tz, 1900 + mtm.tm_year, mtm.tm_mon, mtm.tm_mday, 23, 00 , 00);
    gtk_calendar_clear_marks (GTK_CALENDAR(gde->calendar));
    gtk_calendar_select_day (GTK_CALENDAR(gde->calendar), date);
    g_date_time_unref (date);
...
gnc_date_edit_set_time_internal (GNCDateEdit *gde, time64 the_time)
{
    ...
    {
        GTimeZone *tz = (GTimeZone *)g_time_zone_new_local ();
        GDateTime *date = g_date_time_new (tz, 1900 + mytm->tm_year,
                                               mytm->tm_mon + 1,
                                               mytm->tm_mday,
                                               23, 00 , 00);
        gtk_calendar_clear_marks (GTK_CALENDAR(gde->calendar));
...
gnc_date_edit_init (GNCDateEdit *gde)
{
    ...
    gtk_widget_set_hexpand (GTK_WIDGET(gde), TRUE);
...
gnc_date_edit_dispose (GObject *object)
{
    ...
    gtk_box_remove (GTK_BOX(gde), GTK_WIDGET(gde->date_entry));
    gde->date_entry = NULL;

    gtk_box_remove (GTK_BOX(gde), GTK_WIDGET(gde->date_button));
    gde->date_button = NULL;

    gtk_box_remove (GTK_BOX(gde), GTK_WIDGET(gde->time_entry));
    gde->time_entry = NULL;

    gtk_box_remove (GTK_BOX(gde), GTK_WIDGET(gde->time_combo));
    gde->time_combo = NULL;
...
date_accel_key_press (GtkWidget *widget, const GdkEvent *event, gpointer data)
{
    ...
    if (!gnc_handle_date_accelerator ((GdkEvent*)event, &tm, string))
        return FALSE;
                 ...
                 gpointer user_data)
{
    ...
    if (!date_accel_key_press (widget, event, user_data))
        return FALSE;

    g_signal_stop_emission_by_name (widget, "key-pressed");
    return TRUE;
}

static void
date_focus_out_event (GtkEventControllerFocus *controller, gpointer user_data)
{
    GNCDateEdit *gde = user_data;
    struct tm tm;
...
create_children (GNCDateEdit *gde)
{
    ...
    gboolean show_time = FALSE;

    /* Create the text entry area. */
    gde->date_entry  = gtk_entry_new ();
    gtk_entry_set_max_length (GTK_ENTRY(gde->date_entry), 12);
    gtk_box_append (GTK_BOX(gde), GTK_WIDGET(gde->date_entry));
    gtk_widget_set_visible (GTK_WIDGET(gde->date_entry), TRUE);
    gtk_widget_set_hexpand (GTK_WIDGET(gde->date_entry), TRUE);

    GtkEventController *event_controller_date_entry = gtk_event_controller_key_new ();
    gtk_widget_add_controller (GTK_WIDGET(gde->date_entry), event_controller_date_entry);
    gtk_event_controller_set_propagation_phase (event_controller_date_entry, GTK_PHASE_CAPTURE);
    g_signal_connect (G_OBJECT(event_controller_date_entry), "key-pressed",
                      G_CALLBACK(key_press_entry), gde);

    GtkEventController *focus_controller_date_entry = gtk_event_controller_focus_new ();
    gtk_widget_add_controller (GTK_WIDGET(gde->date_entry), focus_controller_date_entry);
    g_signal_connect (G_OBJECT(focus_controller_date_entry), "leave",
    ...
    gtk_box_append (GTK_BOX(gde), GTK_WIDGET(gde->date_button));

    hbox = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 3);
    gtk_box_set_homogeneous (GTK_BOX(hbox), FALSE);
    gtk_button_set_child (GTK_BUTTON(gde->date_button), GTK_WIDGET(hbox));
    ...
    gtk_box_append (GTK_BOX(hbox), GTK_WIDGET(gde->cal_label));

    if (gde->flags & GNC_DATE_EDIT_SHOW_TIME)
        show_time = TRUE;

    gtk_widget_set_visible (GTK_WIDGET(gde->cal_label), show_time);
    ...
    gtk_widget_set_hexpand (GTK_WIDGET(gde->time_entry), FALSE);
    gtk_widget_set_halign (GTK_WIDGET(gde->time_entry), GTK_ALIGN_START);
    gtk_box_append (GTK_BOX(gde), GTK_WIDGET(gde->time_entry));
    ...
    gtk_widget_set_hexpand (GTK_WIDGET(gde->time_combo), FALSE);
    gtk_widget_set_halign (GTK_WIDGET(gde->time_combo), GTK_ALIGN_START);
    gtk_box_append (GTK_BOX(gde), GTK_WIDGET(gde->time_combo));
    ...
    gtk_widget_set_visible (GTK_WIDGET(gde->time_entry), show_time);
    gtk_widget_set_visible (GTK_WIDGET(gde->time_combo), show_time);

    gde->cal_popup = gtk_popover_new ();
    gtk_widget_set_parent (GTK_WIDGET(gde->cal_popup), GTK_WIDGET(gde->date_button));
    gtk_popover_set_position (GTK_POPOVER(gde->cal_popup), GTK_POS_TOP);
    gtk_widget_set_name (GTK_WIDGET(gde->cal_popup), "gnc-date-edit-popup-window");
    g_signal_connect (G_OBJECT(gde->cal_popup), "closed",
                      G_CALLBACK(delete_popup), gde);

    gde->calendar = gtk_calendar_new ();
    gtk_calendar_set_show_day_names (GTK_CALENDAR(gde->calendar), TRUE);
    gtk_calendar_set_show_week_numbers (GTK_CALENDAR(gde->calendar), FALSE);
    gtk_calendar_set_show_heading (GTK_CALENDAR(gde->calendar), TRUE);

    GtkEventController *event_controller_calendar = gtk_event_controller_key_new ();
    gtk_widget_add_controller (GTK_WIDGET(gde->calendar), event_controller_calendar);
    g_signal_connect (G_OBJECT(event_controller_calendar), "key-pressed",
                      G_CALLBACK(key_press_popup), gde);

    GtkGesture *event_gesture_calendar = gtk_gesture_click_new ();
    gtk_widget_add_controller (GTK_WIDGET(gde->calendar), GTK_EVENT_CONTROLLER(event_gesture_calendar));
    gtk_gesture_single_set_button (GTK_GESTURE_SINGLE(event_gesture_calendar), 1);
    g_signal_connect (G_OBJECT(event_gesture_calendar), "pressed",
                      G_CALLBACK(day_selected_double_click), gde);

    g_signal_connect (G_OBJECT(gde->calendar), "day-selected",
                      G_CALLBACK(day_selected), gde);

    gtk_popover_set_child (GTK_POPOVER(gde->cal_popup), GTK_WIDGET(gde->calendar));
}
