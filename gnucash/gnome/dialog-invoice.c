                                  ...
                                  const gchar *report_guid)
{
    ...
    if (scm_is_false (scm_call_1 (is_invoice_guid, scm_guid)))
        report_guid = PRINTABLE_INVOICE_GUID; // fallback if the option lookup failed
...
gnc_invoice_window_printCB (GtkWindow* parent, gpointer data)
{
    ...
    {
        const gchar *report_guid = use_default_report_template_or_change (parent);

        if (!report_guid)
            return;

        iw->reportPage = gnc_invoice_window_print_invoice
            (parent, iw_get_invoice (iw), report_guid);
    }
