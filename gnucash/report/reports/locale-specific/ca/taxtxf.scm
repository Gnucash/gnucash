...
(define (process-currency-conversion split
                                     ...
                                     print-info
                                     neg?)
;; called if account-commodity does not equal CAD-currency or if
;;    trans-currency not equal CAD-currency
;; if trans-currency = CAD-currency and account-commodity does not equal
;;    CAD-currency, use split value & transaction rate
;; if account-commodity = CAD-currency and trans-currency does not equal
;;    CAD-currency, use split amount & inverse of transaction rate
;; if neither trans-currency nor account-commodity = CAD-currency,
...
;;   a list
  ...
  (let*
    ((splt-value (if (and split
                          (or (gnc-commodity-equiv trans-currency CAD-currency)
     ...
     (amount (if (gnc-commodity-equiv trans-currency CAD-currency)
                 splt-value
                 (if (gnc-commodity-equiv account-commodity CAD-currency)
                     (if neg?
                         (gnc-numeric-neg splt-rpt-amount)
                         splt-rpt-amount)
                     ;; otherwise lookup from pricedb
                     (let ((pricedb (gnc-pricedb-get-db (gnc-get-current-book)))
                          ) ;; if we can convert to CAD
     ...
     (conversion-text (if missing-pricedb-entry?
                          (string-append
                            ...
                            " "
                            converted-qty
                            " to CAD. Go to the Tools->Price Editor dialog to enter prices. Set to zero for this report.)"
                          )
                          (string-append
                            "(Converted "
...
(define (process-transaction-multi-transfer-detail split parent
            CAD-currency full-names? trans-date trans-currency acct-type
  ...
  (let* ((all-tran-splits (xaccTransGetSplitList parent))
         ...
         (output '())
         (trans-rpt-currency-total (gnc-numeric-zero)) ;;for CAD-currency
         (cap-gains-detail-table (gnc:make-html-table))
         (trans-sub-heading-table (gnc:make-html-table))
         (trans-sub-table (gnc:make-html-table))
         (trans-cap-gain-sales-CAD-total (gnc-numeric-zero))
         (trans-cap-gain-basis-CAD-total (gnc-numeric-zero))
        )
        (if (= 4 format_type)
            (begin
               (gnc:html-table-set-style! cap-gains-detail-table "table"
                 ...
                 (let* ((split-acct (xaccSplitGetAccount tran-split))
                       ...
                       (if (and (= 4 format_type) (gnc-numeric-negative-p
                           ...
                           (begin
                              ...
                              (set! trans-cap-gain-sales-CAD-total
                                          (gnc-numeric-add-fixed
                                              trans-cap-gain-sales-CAD-total
                                              (gnc-numeric-neg splt-print-amnt))
                              )
;                              (set! trans-cap-gain-basis-CAD-total #f)
                           )
                       )
        ...
        (list trans-cap-gain-sales-CAD-total
              trans-cap-gain-basis-CAD-total
              output)
  ) ;; end of let*
)
...
(define (process-account-transaction-detail table account split-list
                ...
                tax-mode? show-TXF-data? CAD-currency account-type
  ...
  (let*
    ((account-commodity (xaccAccountGetCommodity account))
     ...
     (account-CAD-total (gnc-numeric-zero))
     (account-cap-gain-sales-CAD-total (gnc-numeric-zero))
     (account-cap-gain-basis-CAD-total (gnc-numeric-zero))
          ...
          (if (or (not (gnc-numeric-zero-p beg-bal-acct-curr))
              ...
              (let* ((curr-conv-note "")
                     ...
                     (curr-conv-data (if (gnc-commodity-equiv
                                                 account-commodity CAD-currency)
                                         curr-conv-data
                                         (process-currency-conversion
                                           #f
                                           CAD-currency
                    ...
                    (set! account-CAD-total (gnc-numeric-add-fixed
                                                  account-CAD-total print-amnt))
              );; end of let*
          );; end of if
        );; end of begin
        #f
           ...
           (let* ((parent (xaccSplitGetParent split))
                 ...
                 (if (or (and transaction-details? tax-mode?
                     ...
                     (let ((cap-gain-data
                          ...
                          (set! account-cap-gain-sales-CAD-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-sales-CAD-total
                                              (car cap-gain-data))
                          )
                          (set! account-cap-gain-basis-CAD-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-basis-CAD-total
                                              (cadr cap-gain-data))
                          )
    ...
    (if tax-mode?
        (let* ((amnt-acct-curr (xaccPrintAmount account-commodity-total
                                                print-info))
               (account-total-amount (xaccPrintAmount account-CAD-total
                                                print-info))
              ...
              (render-total-row table
                                ...
                                (if (gnc-numeric-negative-p account-CAD-total)
                                    #t
                                    #f)
                                (if (= format_type 4)
                                    #t
    ...
    (list account-CAD-total
          output
          (if (or (and (gnc-numeric-positive-p account-CAD-total)
                       (gnc-numeric-positive-p account-commodity-total-as-dr))
                  (and (gnc-numeric-negative-p account-CAD-total)
                       (gnc-numeric-negative-p account-commodity-total-as-dr)))
              account-CAD-total
              (gnc-numeric-neg account-CAD-total))
          account-cap-gain-sales-CAD-total
          account-cap-gain-basis-CAD-total
    )
  ) ;;end of let*
...
;; subtracted in setting from-value?
             ...
             (if (or (and (or (eq? account-type ACCT-TYPE-INCOME)
                 ...
                 (begin
                    ...
                    (let* ((tran-output (process-account-transaction-detail
                           ...
                           (account-CAD-total-as-dr (caddr tran-output))
                          )
                          (set! account-CAD-total (car tran-output))
                          (set! account-cap-gain-sales-CAD-total
                                                    (cadddr tran-output))
                          (set! account-cap-gain-basis-CAD-total
    ...
    (let ((from-date  (gnc-print-time64 from-value "%Y-%b-%d"))
          ...
          (form-sched-line-CAD-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-sales-CAD-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-basis-CAD-total (gnc-numeric-zero))
          (tax-code-sub-item-CAD-total (gnc-numeric-zero))
          (tax-code-sub-item-CAD-total-as-dr (gnc-numeric-zero))
          (tax-code-CAD-total (gnc-numeric-zero))
          (tax-code-cap-gain-sales-CAD-total (gnc-numeric-zero))
          (tax-code-cap-gain-basis-CAD-total (gnc-numeric-zero))
          (tax-code-CAD-total-as-dr (gnc-numeric-zero))
          (saved-tax-code-text "")
            ...
            (let* ((current-form-schedule (caar form-line-acct))
                          ...
                          (begin
                            (if tax-mode?
                                ...
                                (if (gnc-numeric-zero-p
                                              tax-code-sub-item-CAD-total-as-dr)
                                    #t ;; do nothing
                                    (begin
                                      (set! output
                                            (list (render-txf-account
                                              ...
                                              prior-account-copy
                                              tax-entity-type #f)))
                                      (set! tax-code-CAD-total (gnc-numeric-zero))
                                      (set! tax-code-CAD-total-as-dr
                                                             (gnc-numeric-zero))
                            ...
                            (set! tax-code-sub-item-CAD-total
                                                             (gnc-numeric-zero))
                            (set! tax-code-sub-item-CAD-total-as-dr
                                                             (gnc-numeric-zero))
                          )
                          #f ;; else do nothing
                      )
                  ...
                  (if (string=? prior-tax-code "")
                      ...
                      (if tax-mode?
                             ...
                             (if (and suppress-0? (gnc-numeric-zero-p
                                 ...
                                 (let* ((print-info (gnc-account-print-info
                                        ...
                                        (tax-code-cap-gain-sales-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-sales-CAD-total
                                                     print-info))
                                        (tax-code-cap-gain-basis-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-basis-CAD-total
                                                     print-info))
                                       )
                                       ...
                                       (set! tax-code-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-sales-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-basis-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-CAD-total-as-dr
                                                             (gnc-numeric-zero))
                                       (set! tax-code-sub-item-CAD-total
                         ...
                         ;; txf output processing
                         (if (gnc-numeric-zero-p tax-code-CAD-total-as-dr)
                             #t ;; do nothing
                  ...
                  (if (string=? prior-form-sched-line "")
                      (set! form-sched-line-CAD-total (gnc-numeric-zero))
                      (if tax-mode?
                         ;; printed report processing
                             ...
                             (if (and suppress-0? (gnc-numeric-zero-p
                                 ...
                                 (let* ((print-info (gnc-account-print-info
                                        ...
                                        (form-sched-line-cap-gain-sales-total-amount
                                          (xaccPrintAmount
                                             form-sched-line-cap-gain-sales-CAD-total
                                             print-info))
                                       ...
                                       (set! form-sched-line-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-sales-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-basis-CAD-total
                                                             (gnc-numeric-zero))
                                       (set! txf-l-count 0)
                                 )
                             )
                  ...
                  (let* ((account-output (handle-account
                                                 ...
                                                 acct-full-name
                                                 copy))
                         (account-CAD-total-as-dr (caddr account-output))
                        ...
                        (set! tax-code-CAD-total (gnc-numeric-add-fixed
                                                          tax-code-CAD-total
                                                          (car account-output)))
                        (set! tax-code-cap-gain-sales-CAD-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! tax-code-CAD-total-as-dr (gnc-numeric-add-fixed
                                                       tax-code-CAD-total-as-dr
                                                       account-CAD-total-as-dr))
                        (set! tax-code-sub-item-CAD-total (gnc-numeric-add-fixed
                                                     tax-code-sub-item-CAD-total
                                                          (car account-output)))
                        (set! tax-code-sub-item-CAD-total-as-dr
                                         (gnc-numeric-add-fixed
                                             tax-code-sub-item-CAD-total-as-dr
                                                       account-CAD-total-as-dr))
                        (set! form-sched-line-CAD-total (gnc-numeric-add-fixed
                                                       form-sched-line-CAD-total
                                                          (car account-output)))
                        (set! form-sched-line-cap-gain-sales-CAD-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! form-sched-line-cap-gain-basis-CAD-total
                              (gnc-numeric-add-fixed
                                       form-sched-line-cap-gain-basis-CAD-total
                                       (car (cddddr account-output))))
                        (set! need-form-line-acct-header? #f)
                        (set! need-form-schedule-header? #f)
                        (set! need-tax-code-header? #f)
      ...
      (if (not tax-mode?) ; Do Txf mode
                   ...
                   (lambda (port)
                     (gnc:display-report-list-item
                      (list
                       ...
                       (if (or (zero? tax-code-CAD-total) (not prior-account))
                           '()
                           (render-txf-account
                            prior-account
                            (if (= 4 (get-acct-txf-info
          ...
          (begin  ; else do tax report
                    ...
                    (gnc:html-markup
                     ...
                     (gnc:html-markup/format
                      (string-append (if (and (gnc-get-current-book-tax-name)
                                         ...
                                         "Tax Name: ~a<br/>"
                                         "~a")
                      "Period from ~a to ~a<br/>Tax Year ~a<br/>Tax Entity Type: ~a<br/>All amounts in CAD unless otherwise noted")
                 ...
                 (if (and (or (eqv? (get-acct-txf-info 'pns
                     ...
                     (if (and suppress-0? (gnc-numeric-zero-p
                         ...
                         (let* ((print-info (gnc-account-print-info
                                                              prior-account #f))
                                (tax-code-sub-item-total-amount
                                   (xaccPrintAmount tax-code-sub-item-CAD-total
                                                                    print-info))
                               )
                               (render-total-row
                                           table
             ...
             (if (or (and suppress-0? (gnc-numeric-zero-p tax-code-CAD-total))
                     (null? selected-accounts)
                     (not prior-account))
                 #t ;; do nothing
             ...
             (if (not (string=? prior-form-sched-line ""))
                 (if (or (and suppress-0?
                                 (gnc-numeric-zero-p form-sched-line-CAD-total))
                         (null? selected-accounts)
                         (not prior-account)
                     )
                     #t ;; do nothing
                     (let* ((print-info (gnc-account-print-info prior-account
                            ...
                            (form-sched-line-total-amount
                                      (xaccPrintAmount
                                          form-sched-line-CAD-total print-info))
                            (form-sched-line-cap-gain-sales-total-amount
                                      (xaccPrintAmount
                                        form-sched-line-cap-gain-sales-CAD-total
                                                                    print-info))
...
(gnc:define-report
 'version 1
 'name reportname
 'report-guid "6a45fa97877349cd334f9c2192a55fee"
