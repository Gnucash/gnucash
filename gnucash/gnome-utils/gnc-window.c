...
GHashTable *
gnc_window_get_display_hash_table (GncWindow *window)
{
    g_return_val_if_fail (GNC_WINDOW(window), NULL);

    /* optional */
    if (GNC_WINDOW_GET_IFACE(window)->get_disp_hash_table == NULL)
        return NULL;

    return GNC_WINDOW_GET_IFACE(window)->get_disp_hash_table (window);
}
