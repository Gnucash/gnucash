;; -*-scheme-*-
;; Shared implementation for the US and Canadian tax reports.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (process-currency-conversion split
                                     ...
                                     print-info
                                     neg?)
;; called if account-commodity does not equal report-currency or if
;;    trans-currency not equal report-currency
;; if trans-currency = report-currency and account-commodity does not equal
;;    report-currency, use split value & transaction rate
;; if account-commodity = report-currency and trans-currency does not equal
;;    report-currency, use split amount & inverse of transaction rate
;; if neither trans-currency nor account-commodity = report-currency,
...
;;   a list
  ...
  (let*
    ((splt-value (if (and split
                          (or (gnc-commodity-equiv trans-currency report-currency)
     ...
     (amount (if (gnc-commodity-equiv trans-currency report-currency)
                 splt-value
                 (if (gnc-commodity-equiv account-commodity report-currency)
                     (if neg?
                         (gnc-numeric-neg splt-rpt-amount)
                         splt-rpt-amount)
                     ;; otherwise lookup from pricedb
                     (let ((pricedb (gnc-pricedb-get-db (gnc-get-current-book)))
                          ) ;; if we can convert to report-currency
     ...
     (conversion-text (if missing-pricedb-entry?
                          (string-append
                            ...
                            " "
                            converted-qty
                            " to " report-currency-code
                            ". Go to the Tools->Price Editor dialog to enter prices. Set to zero for this report.)"
                          )
                          (string-append
                            "(Converted "
...
(define (process-transaction-multi-transfer-detail split parent
            report-currency full-names? trans-date trans-currency acct-type
  ...
  (let* ((all-tran-splits (xaccTransGetSplitList parent))
         ...
         (output '())
         (trans-rpt-currency-total (gnc-numeric-zero)) ;;for report-currency
         (cap-gains-detail-table (gnc:make-html-table))
         (trans-sub-heading-table (gnc:make-html-table))
         (trans-sub-table (gnc:make-html-table))
         (trans-cap-gain-sales-report-currency-total (gnc-numeric-zero))
         (trans-cap-gain-basis-report-currency-total (gnc-numeric-zero))
        )
        (if (= 4 format_type)
            (begin
               (gnc:html-table-set-style! cap-gains-detail-table "table"
                 ...
                 (let* ((split-acct (xaccSplitGetAccount tran-split))
                       ...
                       (if (and (= 4 format_type) (gnc-numeric-negative-p
                           ...
                           (begin
                              ...
                              (set! trans-cap-gain-sales-report-currency-total
                                          (gnc-numeric-add-fixed
                                              trans-cap-gain-sales-report-currency-total
                                              (gnc-numeric-neg splt-print-amnt))
                              )
;                              (set! trans-cap-gain-basis-report-currency-total #f)
                           )
                       )
        ...
        (list trans-cap-gain-sales-report-currency-total
              trans-cap-gain-basis-report-currency-total
              output)
  ) ;; end of let*
)
...
(define (process-account-transaction-detail table account split-list
                ...
                tax-mode? show-TXF-data? report-currency account-type
  ...
  (let*
    ((account-commodity (xaccAccountGetCommodity account))
     ...
     (account-report-currency-total (gnc-numeric-zero))
     (account-cap-gain-sales-report-currency-total (gnc-numeric-zero))
     (account-cap-gain-basis-report-currency-total (gnc-numeric-zero))
          ...
          (if (or (not (gnc-numeric-zero-p beg-bal-acct-curr))
              ...
              (let* ((curr-conv-note "")
                     ...
                     (curr-conv-data (if (gnc-commodity-equiv
                                                 account-commodity report-currency)
                                         curr-conv-data
                                         (process-currency-conversion
                                           #f
                                           report-currency
                    ...
                    (set! account-report-currency-total (gnc-numeric-add-fixed
                                                  account-report-currency-total print-amnt))
              );; end of let*
          );; end of if
        );; end of begin
        #f
           ...
           (let* ((parent (xaccSplitGetParent split))
                 ...
                 (if (or (and transaction-details? tax-mode?
                     ...
                     (let ((cap-gain-data
                          ...
                          (set! account-cap-gain-sales-report-currency-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-sales-report-currency-total
                                              (car cap-gain-data))
                          )
                          (set! account-cap-gain-basis-report-currency-total
                                          (gnc-numeric-add-fixed
                                              account-cap-gain-basis-report-currency-total
                                              (cadr cap-gain-data))
                          )
    ...
    (if tax-mode?
        (let* ((amnt-acct-curr (xaccPrintAmount account-commodity-total
                                                print-info))
               (account-total-amount (xaccPrintAmount account-report-currency-total
                                                print-info))
              ...
              (render-total-row table
                                ...
                                (if (gnc-numeric-negative-p account-report-currency-total)
                                    #t
                                    #f)
                                (if (= format_type 4)
                                    #t
    ...
    (list account-report-currency-total
          output
          (if (or (and (gnc-numeric-positive-p account-report-currency-total)
                       (gnc-numeric-positive-p account-commodity-total-as-dr))
                  (and (gnc-numeric-negative-p account-report-currency-total)
                       (gnc-numeric-negative-p account-commodity-total-as-dr)))
              account-report-currency-total
              (gnc-numeric-neg account-report-currency-total))
          account-cap-gain-sales-report-currency-total
          account-cap-gain-basis-report-currency-total
    )
  ) ;;end of let*
...
(define (generate-tax-schedule report-name
  ...
  (define report-currency (gnc-commodity-table-lookup
...
;; subtracted in setting from-value?
             ...
             (if (or (and (or (eq? account-type ACCT-TYPE-INCOME)
                 ...
                 (begin
                    ...
                    (let* ((tran-output (process-account-transaction-detail
                           ...
                           (account-report-currency-total-as-dr (caddr tran-output))
                          )
                          (set! account-report-currency-total (car tran-output))
                          (set! account-cap-gain-sales-report-currency-total
                                                    (cadddr tran-output))
                          (set! account-cap-gain-basis-report-currency-total
                            ...
                            (if (or (txf-special-split? tax-code)
                                ...
                                (if (not tax-mode?)
                                    (render-txf-account account
                                            account-report-currency-total-as-dr
                                                  #f #f #t from-value
                                                  account-type tax-code copy
                                                  tax-entity-type #f)
                                    '()))
    ...
    (let ((from-date  (gnc-print-time64 from-value "%Y-%b-%d"))
          ...
          (form-sched-line-report-currency-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-sales-report-currency-total (gnc-numeric-zero))
          (form-sched-line-cap-gain-basis-report-currency-total (gnc-numeric-zero))
          (tax-code-sub-item-report-currency-total (gnc-numeric-zero))
          (tax-code-sub-item-report-currency-total-as-dr (gnc-numeric-zero))
          (tax-code-report-currency-total (gnc-numeric-zero))
          (tax-code-cap-gain-sales-report-currency-total (gnc-numeric-zero))
          (tax-code-cap-gain-basis-report-currency-total (gnc-numeric-zero))
          (tax-code-report-currency-total-as-dr (gnc-numeric-zero))
          (saved-tax-code-text "")
         ...
         (define (handle-tax-code form-line-acct)
            (let* ((current-form-schedule (caar form-line-acct))
                          ...
                          (begin
                            (if tax-mode?
                                ...
                                (if (and suppress-0? (gnc-numeric-zero-p
                                                   tax-code-sub-item-report-currency-total))
                                    #t ;; do nothing
                                ...
                                (if (gnc-numeric-zero-p
                                              tax-code-sub-item-report-currency-total-as-dr)
                                    #t ;; do nothing
                                    (begin
                                      (set! output
                                            (list (render-txf-account
                                              ...
                                              prior-account-copy
                                              tax-entity-type #f)))
                                      (set! tax-code-report-currency-total (gnc-numeric-zero))
                                      (set! tax-code-report-currency-total-as-dr
                                                             (gnc-numeric-zero))
                            ...
                            (set! tax-code-sub-item-report-currency-total
                                                             (gnc-numeric-zero))
                            (set! tax-code-sub-item-report-currency-total-as-dr
                                                             (gnc-numeric-zero))
                          )
                          #f ;; else do nothing
                      )
                  ...
                  (if (string=? prior-tax-code "")
                      ...
                      (if tax-mode?
                         ...
                         (if (and (string=? prior-tax-code current-tax-code)
                             ...
                             #t ;; do nothing
                             (if (and suppress-0? (gnc-numeric-zero-p
                                                            tax-code-report-currency-total))
                                 #t ;; do nothing
                                 (let* ((print-info (gnc-account-print-info
                                                              prior-account #f))
                                        ...
                                        (tax-code-cap-gain-sales-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-sales-report-currency-total
                                                     print-info))
                                        (tax-code-cap-gain-basis-total-amount
                                               (xaccPrintAmount
                                                     tax-code-cap-gain-basis-report-currency-total
                                                     print-info))
                                       )
                                       ...
                                       (set! tax-code-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-sales-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-cap-gain-basis-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! tax-code-report-currency-total-as-dr
                                                             (gnc-numeric-zero))
                                       (set! tax-code-sub-item-report-currency-total
                         ...
                         ;; txf output processing
                         (if (gnc-numeric-zero-p tax-code-report-currency-total-as-dr)
                             #t ;; do nothing
                                    ...
                                    (set! output
                                          (list (render-txf-account
                                              ...
                                              (if (= 4 (get-acct-txf-info
                                                  ...
                                                  (gnc-numeric-neg
                                                   tax-code-cap-gain-sales-report-currency-total)
                                                  tax-code-report-currency-total-as-dr)
                                              #f #f #f #f
                                              (xaccAccountGetType prior-account)
                                              (string->symbol prior-tax-code)
                                              prior-account-copy
                  ...
                  (if (string=? prior-form-sched-line "")
                      (set! form-sched-line-report-currency-total (gnc-numeric-zero))
                      (if tax-mode?
                         ;; printed report processing
                         (if (and (string=? prior-form-sched-line
                             ...
                             #t ;; do nothing
                             (if (and suppress-0? (gnc-numeric-zero-p
                                                     form-sched-line-report-currency-total))
                                 #t ;; do nothing
                                 (let* ((print-info (gnc-account-print-info
                                                              prior-account #f))
                                        ...
                                        (form-sched-line-cap-gain-sales-total-amount
                                          (xaccPrintAmount
                                             form-sched-line-cap-gain-sales-report-currency-total
                                             print-info))
                                       ...
                                       (set! form-sched-line-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-sales-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! form-sched-line-cap-gain-basis-report-currency-total
                                                             (gnc-numeric-zero))
                                       (set! txf-l-count 0)
                                 )
                             )
                  ...
                  (let* ((account-output (handle-account
                                                 ...
                                                 acct-full-name
                                                 copy))
                         (account-report-currency-total-as-dr (caddr account-output))
                        ...
                        (set! tax-code-report-currency-total (gnc-numeric-add-fixed
                                                          tax-code-report-currency-total
                                                          (car account-output)))
                        (set! tax-code-cap-gain-sales-report-currency-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! tax-code-report-currency-total-as-dr (gnc-numeric-add-fixed
                                                       tax-code-report-currency-total-as-dr
                                                       account-report-currency-total-as-dr))
                        (set! tax-code-sub-item-report-currency-total (gnc-numeric-add-fixed
                                                     tax-code-sub-item-report-currency-total
                                                          (car account-output)))
                        (set! tax-code-sub-item-report-currency-total-as-dr
                                         (gnc-numeric-add-fixed
                                             tax-code-sub-item-report-currency-total-as-dr
                                                       account-report-currency-total-as-dr))
                        (set! form-sched-line-report-currency-total (gnc-numeric-add-fixed
                                                       form-sched-line-report-currency-total
                                                          (car account-output)))
                        (set! form-sched-line-cap-gain-sales-report-currency-total
                              (gnc-numeric-add-fixed
                        ...
                        (set! form-sched-line-cap-gain-basis-report-currency-total
                              (gnc-numeric-add-fixed
                                       form-sched-line-cap-gain-basis-report-currency-total
                                       (car (cddddr account-output))))
                        (set! need-form-line-acct-header? #f)
                        (set! need-form-schedule-header? #f)
                        (set! need-tax-code-header? #f)
      ...
      (if (not tax-mode?) ; Do Txf mode
                   ...
                   (lambda (port)
                     (gnc:display-report-list-item
                      (list
                       ...
                       (if (or (zero? tax-code-report-currency-total) (not prior-account))
                           '()
                           (render-txf-account
                            prior-account
                            (if (= 4 (get-acct-txf-info
          ...
          (begin  ; else do tax report
                    ...
                    (gnc:html-markup
                     ...
                     (gnc:html-markup/format
                      (string-append (if (and (gnc-get-current-book-tax-name)
                                         ...
                                         "Tax Name: ~a<br/>"
                                         "~a")
                      "Period from ~a to ~a<br/>Tax Year ~a<br/>Tax Entity Type: ~a<br/>All amounts in ~a unless otherwise noted")
                 ...
                 (if (and (or (eqv? (get-acct-txf-info 'pns
                     ...
                     (if (and suppress-0? (gnc-numeric-zero-p
                         ...
                         (let* ((print-info (gnc-account-print-info
                                                              prior-account #f))
                                (tax-code-sub-item-total-amount
                                   (xaccPrintAmount tax-code-sub-item-report-currency-total
                                                                    print-info))
                               )
                               (render-total-row
                                           table
             ...
             (if (or (and suppress-0? (gnc-numeric-zero-p tax-code-report-currency-total))
                     (null? selected-accounts)
                     (not prior-account))
                 #t ;; do nothing
             ...
             (if (not (string=? prior-form-sched-line ""))
                 (if (or (and suppress-0?
                                 (gnc-numeric-zero-p form-sched-line-report-currency-total))
                         (null? selected-accounts)
                         (not prior-account)
                     )
                     #t ;; do nothing
                     (let* ((print-info (gnc-account-print-info prior-account
                            ...
                            (form-sched-line-total-amount
                                      (xaccPrintAmount
                                          form-sched-line-report-currency-total print-info))
                            (form-sched-line-cap-gain-sales-total-amount
                                      (xaccPrintAmount
                                        form-sched-line-cap-gain-sales-report-currency-total
                                                                    print-info))
