...
gnc_file_ofx_import_process_file (ofx_info* info)
{
...
#ifdef G_OS_WIN32
    char * selected_filename = g_win32_locale_filename_from_utf8 (filename);
    g_free (filename);
#else
    char * selected_filename = filename;
#endif
    ...
    else
    {
        ...
        gnc_gen_trans_list_show_reconcile_after_close (info->gnc_ofx_importer_gui, info->statement != NULL);
...
void gnc_file_ofx_import (GtkWindow *parent)
{
    ...
    if (selected_filenames)
    {
        ...
        ofx_info* info = g_new(ofx_info,1);
        info->num_trans_processed = 0;
        info->statement = NULL;
        info->parent = parent;
        info->run_reconcile = FALSE;
