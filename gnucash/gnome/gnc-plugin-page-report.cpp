...
{
    { "FilePrintAction", N_("_Print Report"), true, N_("Print the current report") },
    { "FilePrintPDFAction", N_("Export as P_DF..."), true, N_("Export the current report as a PDF document") },
    { "ReportSaveAction", N_("Save _Report Configuration"), true, nullptr },
    { "ReportSaveAsAction", N_("Save Report Configuration As..."), true, nullptr },
    { "ReportExportAction", N_("Export _Report"), true,  N_("Export HTML-formatted report to file") },
...
gnc_plugin_page_report_focus_widget (GncPluginPage *report_plugin_page)
{
    if (GNC_IS_PLUGIN_PAGE_REPORT(report_plugin_page))
    {
        ...
        gnc_main_window_add_sub_menus (GNC_MAIN_WINDOW(report_plugin_page->window), report_plugin_page,
                                       add_submenus, add_submenus_n_actions);

        gnc_main_window_update_action_labels (GNC_MAIN_WINDOW(report_plugin_page->window),
...
gnc_plugin_page_report_menu_updates (GncPluginPage *plugin_page)
{
    GncPluginPageReportPrivate *priv;
    GncPluginPageReport *report;
    GncActionUpdate save_updates[2];
    ...
    save_updates[0] = { "ReportSaveAction", N_("Save Report Configuration"), true, report_save_str };
    save_updates[1] = { "ReportSaveAsAction", N_("Save Report Configuration As..."), true, report_saveas_str };

    /* update the action labels */
    gnc_main_window_update_action_labels (window, save_updates, G_N_ELEMENTS(save_updates));
    ...
    if (priv->webkit2)
    {
        GncActionUpdate pdf_update[1];
        pdf_update[0] = { "FilePrintPDFAction", nullptr, false, report_save_str };
        gnc_main_window_update_action_labels (window, pdf_update, G_N_ELEMENTS(pdf_update));
    }
...
gnc_plugin_page_report_constr_init (GncPluginPageReport *plugin_page, gint reportId)
{
    ...
    gnc_plugin_update_actions (simple_action_group, initially_insensitive_actions,
                               "sensitive", FALSE);
}
