...
gnc_xfer_dialog_update_price (XferDialog * xferData)
{
	...
	if (!xferData)
		return;
	if (!xferData->from_commodity || !xferData->to_commodity)
		return;
	...
	prc = gnc_pricedb_lookup_nearest_in_time (xferData->pricedb, from, to, date);
	...
	if (prc) {
		...
		prc = gnc_pricedb_lookup_nearest_in_time (xferData->pricedb, to, from, date);
		if (!prc)
			return;
		price = gnc_price_get_value (prc);
		...
		price = gnc_numeric_div (gnc_numeric_create (1, 1), price, GNC_DENOM_AUTO, GNC_DENOM_REDUCE);
	}
...
gnc_xfer_dialog_set_price_auto (XferDialog * xferData,
				...
				const gnc_commodity * from_currency, const gnc_commodity * to_currency)
{
	...
	if (!currency_active) {
		...
		gnc_amount_edit_set_amount (GNC_AMOUNT_EDIT (xferData->price_edit), gnc_numeric_zero ());
		entry = GTK_ENTRY (gnc_amount_edit_gtk_entry (GNC_AMOUNT_EDIT (xferData->price_edit)));
	...
	if (!gnc_is_euro_currency (from_currency) || !gnc_is_euro_currency (to_currency))
	...
	price = gnc_numeric_div (from_rate, to_rate, GNC_DENOM_AUTO, GNC_DENOM_SIGFIGS (6) | GNC_RND_ROUND);
...
gnc_xfer_dialog_curr_acct_activate (XferDialog * xferData)
{
	...
	curr_active = (xferData->exch_rate || ((from_account != NULL) && (to_account != NULL)))
	...
	gtk_widget_set_sensitive (xferData->to_amount_edit, curr_active
				  &&
				  gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (xferData->amount_radio)));
	...
	if (!curr_active) {
		...
		gnc_amount_edit_set_amount (GNC_AMOUNT_EDIT (xferData->to_amount_edit), gnc_numeric_zero ());
		entry = GTK_ENTRY (gnc_amount_edit_gtk_entry (GNC_AMOUNT_EDIT (xferData->to_amount_edit)));
		gtk_entry_set_text (entry, "");
	}
}
...
price_amount_radio_toggled_cb (GtkToggleButton * togglebutton, gpointer data)
{
	...
	gtk_widget_set_sensitive (xferData->price_edit,
				  gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (xferData->price_radio)));
...
static void
gnc_xfer_dialog_from_tree_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
	XferDialog *xferData = data;
	...
	gnc_amount_edit_set_print_info (GNC_AMOUNT_EDIT (xferData->amount_edit), print_info);
...
gnc_xfer_dialog_to_tree_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
	...
	commodity = xaccAccountGetCommodity (account);
	gtk_label_set_text (GTK_LABEL (xferData->to_currency_label), gnc_commodity_get_printname (commodity));
	...
	gnc_amount_edit_set_print_info (GNC_AMOUNT_EDIT (xferData->to_amount_edit), print_info);
	gnc_amount_edit_set_fraction (GNC_AMOUNT_EDIT
				      (xferData->to_amount_edit), xaccAccountGetCommoditySCU (account));
...
gnc_parse_error_dialog (XferDialog * xferData, const char *error_string)
{
	...
	gnc_error_dialog_parented (GTK_WINDOW (xferData->dialog),
				   "%s\n\n%s: %s.", error_string, _("Error"), parse_error_string);
}
...
gnc_xfer_dialog_quickfill (XferDialog * xferData)
{
	 ...
	 */
	if ((other = xaccSplitGetOtherSplit (split)) && (other_acct = xaccSplitGetAccount (other))) {
...
gnc_xfer_description_insert_cb (GtkEntry * entry,
				...
				const gint insert_text_len, gint * start_pos, XferDialog * xferData)
{
	...
	/* insert_text is not NULL-terminated, how annoying */
	{
		...
		change_text_len = gdk_mbstowcs (change_text_w, temp, insert_text_len);
	...
	case GDK_ISO_Left_Tab:
		if (!(event->state & GDK_SHIFT_MASK)) {	/* Complete on Tab,
			...
			gtk_old_editable_claim_selection (GTK_OLD_EDITABLE
							  (xferData->description_entry), FALSE, event->time);
		}
		break;
	}
...
static gboolean
gnc_xfer_description_button_release_cb (GtkEntry * entry, GdkEventButton * event, XferDialog * xferData)
{
...
static gboolean
gnc_xfer_amount_update_cb (GtkWidget * widget, GdkEventFocus * event, gpointer data)
{
	XferDialog *xferData = data;
...
gnc_xfer_update_to_amount (XferDialog * xferData)
{
	...
	if (gnc_numeric_zero_p (price))
		...
		to_amount = gnc_numeric_mul (amount, price,
					     xaccAccountGetCommoditySCU (account), GNC_RND_ROUND);

	gnc_amount_edit_set_amount (GNC_AMOUNT_EDIT (xferData->to_amount_edit), to_amount);
...
static gboolean
gnc_xfer_price_update_cb (GtkWidget * widget, GdkEventFocus * event, gpointer data)
{
	XferDialog *xferData = data;
...
static gboolean
gnc_xfer_to_amount_update_cb (GtkWidget * widget, GdkEventFocus * event, gpointer data)
{
	XferDialog *xferData = data;
	gnc_numeric amount, price, to_amount;
	Account *account;
	...
	amount = gnc_amount_edit_get_amount (GNC_AMOUNT_EDIT (xferData->amount_edit));
	to_amount = gnc_amount_edit_get_amount (GNC_AMOUNT_EDIT (xferData->to_amount_edit));
...
gnc_xfer_dialog_select_from_currency (XferDialog * xferData, gnc_commodity * cur)
{
	...
	gtk_label_set_text (GTK_LABEL (xferData->from_currency_label), gnc_commodity_get_printname (cur));
...
{
	gtk_label_set_text (GTK_LABEL (xferData->to_currency_label), gnc_commodity_get_printname (cur));

	gnc_amount_edit_set_fraction (GNC_AMOUNT_EDIT
				      (xferData->to_amount_edit), gnc_commodity_get_fraction (cur));
...
static void
gnc_xfer_dialog_lock_account_tree (XferDialog * xferData, XferDirection direction, gboolean hide)
{
	GtkTreeView *tree_view;
...
gnc_xfer_dialog_set_exchange_rate (XferDialog * xferData, gnc_numeric exchange_rate)
{
	...
	gnc_amount_edit_set_amount (GNC_AMOUNT_EDIT (xferData->price_edit), exchange_rate);
...
gnc_xfer_dialog_response_cb (GtkDialog *dialog, gint response_id, gpointer data)
{
	...
	if (xferData->exch_rate == NULL) {
		...
		if (from_account == to_account) {
			const char *message = _("You can't transfer from and to the same " "account!");
			gnc_error_dialog_parented (GTK_WINDOW (xferData->dialog), message);
			return;
		}

		if (xaccAccountGetPlaceholder (from_account) || xaccAccountGetPlaceholder (to_account)) {
			const char *placeholder_format = _("The account %s\ndoes not allow transactions.\n");
			char *name;

			if (xaccAccountGetPlaceholder (from_account))
				name = xaccAccountGetFullName (from_account, gnc_get_account_separator ());
			else
				name = xaccAccountGetFullName (to_account, gnc_get_account_separator ());
	...
	if (curr_trans) {
		...
		if (!gnc_amount_edit_evaluate (GNC_AMOUNT_EDIT (xferData->to_amount_edit))) {
			if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (xferData->amount_radio))) {
				gnc_parse_error_dialog (xferData, _("You must enter a valid `to' amount."));
				return;
			}
		}
	...
	if (xferData->pricedb) {
		 ...
		 */
		if (!gnc_commodity_equal (from, to) && !(gnc_is_euro_currency (from)
							 && gnc_is_euro_currency (to))) {
			GNCPrice *price;
			GList *prices;
			...
			if (prices) {
				PINFO ("Found price for %s in %s",
				       gnc_commodity_get_mnemonic (from), gnc_commodity_get_mnemonic (to));
			} else {
				prices = gnc_pricedb_lookup_at_time (xferData->pricedb, to, from, ts);
				if (prices) {
					PINFO ("Found reverse price for %s in %s",
					       gnc_commodity_get_mnemonic (to),
					       gnc_commodity_get_mnemonic (from));
				}
			}
			...
			if (prices) {
				...
				if (from == gnc_default_currency ()) {
					...
					value = gnc_numeric_div
						(gnc_numeric_create (1, 1), value, PRECISION, GNC_RND_ROUND);
...
gnc_xfer_dialog_create (GtkWidget * parent, XferDialog * xferData)
{
	...
	/* optional intermediate currency account */
	{
		...
		gnc_amount_edit_set_print_info (GNC_AMOUNT_EDIT (edit), gnc_default_price_print_info ());
...
gnc_xfer_dialog (GtkWidget * parent, Account * initial)
{
	...
	gnc_register_gui_component (DIALOG_TRANSFER_CM_CLASS, NULL, close_handler, xferData);
...
static void
gnc_xfer_dialog_set_account_frame_label (XferDialog * xferData, const gchar * label, XferDirection direction)
{
	if (xferData && label) {
...
void
gnc_xfer_dialog_add_user_specified_button (XferDialog * xferData,
					   const gchar * label, GtkSignalFunc callback, gpointer user_data)
{
...
void
gnc_xfer_dialog_toggle_currency_frame (XferDialog * xferData, gboolean show_frame)
{
 ...
 */
gboolean
gnc_xfer_dialog_run_until_done (XferDialog * xferData)
{
	...
	if (xferData) {
		...
		while (TRUE) {
			...
			if (result_ok == TRUE) {
				...
				if (!gnc_find_first_gui_component
				    (DIALOG_TRANSFER_CM_CLASS, find_xfer, xferData)) {
...
void
gnc_xfer_dialog_quickfill_to_account (XferDialog * xferData, gboolean qf_to_account)
{
