...
(define (split->date s) (gnc-localtime (xaccTransGetDate (xaccSplitGetParent s))))
(define (date->year dt)    (gnc:date-get-year dt))
...
(define date-subtotal-list
  ...
  (list
   ...
   (cons 'monthly (list
                   ...
                   (cons 'text (_ "Monthly"))
                   (cons 'tip (_ "Monthly."))
                   (cons 'renderer-fn (lambda (s) (gnc:date-get-month-year-string (split->date s))))))

   (cons 'quarterly (list
                     (cons 'split-sortvalue (lambda (s) (date->quarter (split->date s))))
                     (cons 'text (_ "Quarterly"))
                     (cons 'tip (_ "Quarterly."))
                     (cons 'renderer-fn (lambda (s) (gnc:date-get-quarter-year-string (split->date s))))))

   (cons 'yearly (list
                  (cons 'split-sortvalue (lambda (s) (date->year (split->date s))))
                  (cons 'text (_ "Yearly"))
                  (cons 'tip (_ "Yearly."))
                  (cons 'renderer-fn (lambda (s) (gnc:date-get-year-string (split->date s))))))))
      ...
      (define comparator-function
        (if (member key DATE-SORTING-TYPES)
            (let ((date (lambda (s)
              ...
              (case date-subtotal
                ((yearly)    (lambda (s) (date->year (gnc-localtime (date s)))))
