...
def process_txt_file(
    ...
    if not skip_check and modified != 1:
        raise RuntimeError(
            f"Expected exactly 1 modified message in {filepath}, but found {modified}"
        )
...
def process_po_file(
    ...
    for entry in po:
        ...
        if entry_ctxt == old_ctxt and entry.msgid == old_id:
            if entry.previous_msgctxt is None:
                entry.previous_msgctxt = entry.msgctxt
            entry.msgctxt = new_ctxt if new_ctxt is not None else ""
            if entry.previous_msgid is None:
                entry.previous_msgid = entry.msgid
            entry.msgid = new_id
...
def main():
    ...
    parser.add_argument(
        "--skip-check",
        action="store_true",
        help="Don't throw error when not exactly 1 match is found",
    )
